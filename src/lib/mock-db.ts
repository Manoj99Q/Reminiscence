...
class MockDatabase {
  ...
  collection<T>(name: string) {
    return {
      ...
      deleteOne: (query: any) => this.deleteOneWithCollection(name, query),
  ...
  private async deleteManyWithCollection(collectionName: string, query: any): Promise<{ deletedCount: number }> {
    ...
    } else if (collection === this.categories) {
      this.categories = filtered as MockCategory[];
    }
    
    return { deletedCount: initialLength - filtered.length };
  }

  private async deleteOneWithCollection(collectionName: string, query: any): Promise<{ deletedCount: number }> {
    const collection = this.getCollectionByName(collectionName);
    if (!collection) return { deletedCount: 0 };
