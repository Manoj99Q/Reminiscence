...
export default function NewEntry() {
    ...
    const loadCategories = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.log('📊 Loaded categories for NewEntry:', dbCategories);
          setCategories(dbCategories);
        } else {
          console.warn('⚠️ Failed to load categories for NewEntry');
          setCategories([]);
        }
      } catch (error) {
        console.error('❌ Error loading categories for NewEntry:', error);
        setCategories([]);
      } finally {
  ...
  // Convert database categories to UI format
  const getIconFromCategory = (icon: string) => {
  ...
  const getColorFromCategory = (color: string) => {
    const colorMap: Record<string, string> = {
      blue: 'bg-blue-100 border-blue-200 hover:bg-blue-50',
      ...
      pink: 'bg-pink-100 border-pink-200 hover:bg-pink-50',
      indigo: 'bg-indigo-100 border-indigo-200 hover:bg-indigo-50',
      red: 'bg-red-100 border-red-200 hover:bg-red-50',
      teal: 'bg-teal-100 border-teal-200 hover:bg-teal-50'
    };
    return colorMap[color] || 'bg-gray-100 border-gray-200 hover:bg-gray-50';
  };

  const journalCollections = categories.map(category => ({
    id: category.id,
    name: category.name,
    icon: getIconFromCategory(category.icon),
    color: getColorFromCategory(category.color),
  ...
  const generateImageFromText = async () => {
    if (!content.trim()) {
      toast.error('Please write some content first to generate an image');
      return;
    }
  ...
  const logDataWithImage = () => {
    ...
    toast.success('Data logged to console! Check browser developer tools.');
  };
      ...
      <div className="max-w-2xl mx-auto px-4 py-16">
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div>
              ...
              {isLoadingCategories ? (
                ...
                <div className="text-center py-8 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                  ...
                  <h4 className="text-lg font-medium text-gray-900 mb-2">No Collections Available</h4>
                  <p className="text-gray-600">Please create collections in the dashboard first.</p>
                </div>
              ) : (
            ...
            <button
              type="submit"
              disabled={isSubmitting || !content.trim() || !selectedCollection || journalCollections.length === 0}
